...
# -*- coding: utf-8 -*-
"""
五子各得 (오자각득 / 천수용오도) 배치 2 — 현대 조합론·위치 분석

원본 배치(analyze_ojagakdeuk.py)와 모양은 동일하되 수 배치가 다른 변형.
중앙선(위→아래): 12, 5, 2, 15, 6, 1, 16
좌측(바깥 열→안쪽 열, 세로): 7 / 13, 3, 11 / 17, 20, 10
우측(바깥 열→안쪽 열, 세로): 8 / 19, 4, 14 / 18, 9, 21
분석 대상: 1~21 전부(21개 수)를 천수용오도 형태로 배치한 도상.
...
RESIDUE_STYLE: dict[int, dict[str, str]] = {
    1: {"face": "#CFE2F5", "edge": "#4488CC", "name": "Water", "ko": "수"},
    2: {"face": "#F4D0D0", "edge": "#CC4444", "name": "Fire", "ko": "화"},
    3: {"face": "#D4ECD4", "edge": "#44AA44", "name": "Wood", "ko": "목"},
    4: {"face": "#E8E8E8", "edge": "#888888", "name": "Metal", "ko": "금"},
    0: {"face": "#F6E5A3", "edge": "#CC9944", "name": "Earth", "ko": "토"},
}
...
def validate() -> None:
    ...
    assert set(all_values) == set(range(1, 22)), "1~21 전부 사용"
    assert sum(all_values) == 231, "전체 합은 231"
...
print("=" * 60)
print("五子各得 (오자각득 / 천수용오도) 배치 2 — 현대 조합론·위치 분석")
print("=" * 60)
...
draw_nodes(ax)
ax.set_title(
    "五子各得 (오자각득 / 천수용오도) 배치 2 - 원본 배치\n"
    "1~21 · mod 5 오행 분류 · 천수용오도 기하 구조",
    fontsize=16,
    fontweight="bold",
)
ax.set_xlim(-5.0, 5.0)
...
plt.suptitle("오행(五行)별 서브그룹 분해 (배치 2)", fontsize=16, fontweight="bold", y=1.02)
plt.tight_layout()
...
ax.set_title("mod 5 잉여 공간 분포 (배치 2)", fontsize=13, fontweight="bold")
...
ax.set_title("오행별 좌표 분포 (배치 2)", fontsize=13, fontweight="bold")
...
ax.set_title("좌·중·우 대칭 분포 (배치 2)", fontsize=13, fontweight="bold")
ax.axis("off")
...
ax.set_title("수평 단별 합 (배치 2)", fontsize=13, fontweight="bold")
...
ax.set_title("오행별 수 합 (55, 38, 42, 46, 50)", fontsize=12, fontweight="bold")
...
ax.set_title("좌·중·우 대칭 합 (좌=81, 중=57, 우=93)", fontsize=12, fontweight="bold")
...
ax.set_title("수직 층별 합 분포 (배치 2)", fontsize=13, fontweight="bold")
...
ax.set_title("수평 단별 합 및 노드 수 (배치 2)", fontsize=13, fontweight="bold")
ax.set_ylabel("합", fontsize=10)
...
ax.set_title("좌·중·우 대칭 합 (배치 2)", fontsize=13, fontweight="bold")
ax.set_ylabel("합", fontsize=10)
for bar, val in zip(ax.patches, components.values()):
    ax.text(
        bar.get_x() + bar.get_width() / 2,
