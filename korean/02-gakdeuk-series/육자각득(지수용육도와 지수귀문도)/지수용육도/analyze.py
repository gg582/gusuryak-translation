...
《구수략(九數略)》계열 도상 중 지수용육도를 현대 수학 언어로 재해석.
...
RESIDUE_STYLE: Final = {
    ...
    0: {"label": "n mod 5 ≡ 0", "face": "#F5E1A2", "edge": "#B98D00", "text": "#725500", "name": "Earth", "ko": "토"},
}
...
def validate() -> None:
    ...
    for hex_name, vertices in HEXAGONS.items():
        ...
        if value_sum != TARGET_SUM:
            raise ValueError(
                f"{DISPLAY_LABELS[hex_name]} 육각형의 합은 {value_sum}이며, {TARGET_SUM}이어야 합니다."
            )
...
print("\n육각형별 6자 합:")
for hex_name, vertices in HEXAGONS.items():
    print(f"  {DISPLAY_LABELS[hex_name]}: {' + '.join(map(str, vertices))} = {sum(vertices)}")
...
for hex_name in HEXAGONS:
    ...
    print(f"  {DISPLAY_LABELS[hex_name]}: {counts_ko}")

print("\n정점별 공유 육각형 수:")
for v in range(1, 21):
    hex_labels = [DISPLAY_LABELS[h] for h in VERTEX_TO_HEXAGONS[v]]
    print(f"  {v}({DISPLAY_LABELS[phase_of(v)]}): {shared_count(v)}개 {hex_labels}")
...
for m0, total in FAMILY_RESIDUE:
    ...
    print(f"  M0={m0}({DISPLAY_LABELS[wx]}): 클래스 합={total}")
...
for idx, wx in enumerate(["Water", "Fire", "Wood", "Metal", "Earth"]):
    ax = axes[idx + 1]
    draw_hexagon_boundaries(ax, labels=False)
    wx_nodes = [n for n in range(1, 21) if phase_of(n) == wx]
...
ax.set_title("인접 행렬 (육각형 그래프)", fontsize=13, fontweight="bold")
...
draw_nodes(ax)
ax.set_title("5개 육각형의 6-사이클", fontsize=13, fontweight="bold")
set_ax_lims(ax)
...
ax.bar([DISPLAY_LABELS[h] for h in hex_names], hex_sums, color=hex_bar_colors, edgecolor="black", linewidth=1.5)
...
ax.set_xlabel("사이클 길이", fontsize=10)
...
ax.set_title("차수 (육각형 그래프)", fontsize=12, fontweight="bold")
...
ax = axes[1, 0]
wx_sums = {wx: sum([n for n in range(1, 21) if phase_of(n) == wx]) for wx in ["Water", "Fire", "Wood", "Metal", "Earth"]}
...
ax = axes[1, 1]
components = {
    "upper_left": sum(HEXAGONS["upper_left"]),
    "upper_right": sum(HEXAGONS["upper_right"]),
    "center": sum(HEXAGONS["center"]),
    "lower_left": sum(HEXAGONS["lower_left"]),
    "lower_right": sum(HEXAGONS["lower_right"]),
    "전체": sum(range(1, 21)),
}
ax.bar(
...
ax.set_ylabel("잉여 클래스 합", fontsize=10)
...
ax.set_ylabel("공유 육각형 수", fontsize=10)
...
| 육각형 | 6개 정점 | 합 |
|---|---|---|
| {DISPLAY_LABELS['upper_left']} | {' + '.join(map(str, HEXAGONS['upper_left']))} | {sum(HEXAGONS['upper_left'])} |
| {DISPLAY_LABELS['upper_right']} | {' + '.join(map(str, HEXAGONS['upper_right']))} | {sum(HEXAGONS['upper_right'])} |
| {DISPLAY_LABELS['center']} | {' + '.join(map(str, HEXAGONS['center']))} | {sum(HEXAGONS['center'])} |
| {DISPLAY_LABELS['lower_left']} | {' + '.join(map(str, HEXAGONS['lower_left']))} | {sum(HEXAGONS['lower_left'])} |
| {DISPLAY_LABELS['lower_right']} | {' + '.join(map(str, HEXAGONS['lower_right']))} | {sum(HEXAGONS['lower_right'])} |
...
- **매개 중심성 상위 5개**:
{chr(10).join(f"  - {n}({DISPLAY_LABELS[phase_of(n)]}): {v:.3f}" for n, v in sorted(betw_full.items(), key=lambda x: -x[1])[:5])}
...
지수용육도는 《구수략》 계열의 전통 수학 퍼즐로, 1부터 20까지의 수를 5개 육각형에 배치하여 각 육각형의 합이 63이 되도록 한 도식입니다.
