...
`v(c)+v(-c)=271`은 현재 판독된 來積法과 기하 수치에서 **논리적으로 도출되지 않는다**. 다만 洛書 보수대합, `通加洛書數六倍`, `54+6=60`, 중심대칭 기하, 1…270의 보수 집합, 그리고 《九數略》 병편의 중심 공백 7칸·`一百四十四雙`·`倍積/從方/面數` 도형 수 계산 선례가 정확히 맞물린다. 따라서 이는 임의적 장식이 아닌 **강한 내부 복원 조건**이다.
...
따라서 현 솔버가 사용하는 다른 균형 목표와 위 집계까지 보태어도, 대척보수는 필연이 아니다. 같은 종류의 국소 교환을 조건부 증인해 전체에서 전수 검사하면 514개가 성립한다. 이는 방대한 해공간에서 우연히 얻은 단일 예외가 아니라, 집계 제약이 위치-값 등변성을 대체하지 못한다는 구조적 사실이다.
