...
def pair_order_scan(values: dict, grid: HexGrid) -> dict:
    """나선 순회를 따라 대점쌍의 작은 값이 순서 있게 배정되었는지 검사."""
    slot_of = {}
    ...
    return {
        ...
        "verdict": (
            f"나선 순서 기준 대점쌍 배정의 최장 연속 구간 {longest}쌍 — "
...
def ray_symmetry_scan(values: dict, grid: HexGrid) -> dict:
    ...
    return {
        ...
        "verdict": (
            "마주 보는 광선 차분의 부호 반전은 대점 보수쌍(합 271) 가설의 "
...
def build_reverse_analysis(values: dict, grid: HexGrid,
    ...
    analysis["verdict"] = (
        ...
        "대점쌍 271)은 주석과 완전히 정합하지만, 값의 배치 순서 규칙은 "
...
def write_reverse_markdown(analysis: dict, path: str) -> None:
    ...
    lines = [
        ...
        f"| 대점쌍 건설적 배정 순서 | 나선 순회 기준 연속 구간 | 없음 — 최장 {s['pair_order']['longest_consecutive_run']}쌍 |",
        f"| 광선 차분 규칙 | 마주 보는 광선 비교 | 부호 반전 — 대점쌍 가설의 자동 귀결 |",
