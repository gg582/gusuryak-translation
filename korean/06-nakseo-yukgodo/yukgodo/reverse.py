...
def annotation_crosscheck(analysis: dict) -> list[dict]:
    ...
    return [
        ...
        {
            "fragment": "寄左 / 序左",
            "reading": "배치 순서 지시 (자형 판독 완비)",
            "status": "미해결 (언어적 해석)",
            "evidence": (
                "글자는 명확하나 지정하는 구체적 산학 배치 알고리즘의 해석 문제. "
        ...
        },
        {
            ...
            "status": "미해결 (언어적 해석)",
            "evidence": "자형은 1급 확정 전사되었으나 산학 셈법상의 문맥적 의미 해독 필요",
        },
    ]
...
def build_reverse_analysis(values: dict, grid: HexGrid,
    ...
    analysis["verdict"] = (
        ...
        "글자 자형 전사는 완료되었으나 산학 용어상의 언어적/학술적 해독이 필요하며 최적해가 다수 존재해 "
        "재구 도안만으로는 판정할 수 없다. 확정 가능한 것은 '무엇이 아닌가'의 범위까지이며, "
        "알고리즘 본문의 확정에는 문맥적/언어적 산학 용어 해독이 더 요구된다."
    )
    return analysis
...
def write_reverse_markdown(analysis: dict, path: str) -> None:
    ...
    lines += [
        ...
        "  0/270이므로, 합 조건만으로는 원래 배치를 특정할 수 없고, 원래 배치가",
        "  따른 순서 규칙도 이 표본들에서 복구할 수 없다.",
        "- 寄左/序左/以算遠則係以六 등의 글자 자형 판독은 완료되었으나, 문맥 및 당대 산학 용어로서의 언어적/학술적 완벽한 해독 및 본문 연결은 심도 있는 고증 조사를 필요로 한다.",
        "",
    ]
    with open(path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
