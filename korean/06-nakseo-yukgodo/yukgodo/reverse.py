...
def mod6_scan(values: dict, grid: HexGrid) -> dict:
    """고리별 mod 6 잔여 분포가 균형(각 k개)인지 검사."""
    per_ring = {}
    balanced = True
    for k in range(1, grid.radius + 1):
...
def pair_order_scan(values: dict, grid: HexGrid) -> dict:
    """나선 순회를 따라 점대칭 쌍의 작은 값이 순서 있게 배정되었는지 검사."""
    slot_of = {}
    ...
    return {
        ...
        "verdict": (
            f"나선 순서 기준 점대칭 쌍 배정의 최장 연속 구간 {longest}쌍 — "
...
def ray_symmetry_scan(values: dict, grid: HexGrid) -> dict:
    ...
    return {
        ...
        "verdict": (
            "마주 보는 광선 차분의 부호 반전은 점대칭 보수 쌍(합 271) 가설의 "
...
def annotation_crosscheck(analysis: dict) -> list[dict]:
    ...
    return [
        ...
        },
        {
            ...
            "reading": "500 및 506 수치",
            "status": "보류 (3급)",
            "evidence": "계산 사슬에서 고립되어 있으며, 2-op 생성식 다수로 특정 불가 (NAEJEOK_ASSESSMENT.md 참고)",
        },
        {
            "fragment": "寄左 / 序左",
...
def build_reverse_analysis(values: dict, grid: HexGrid,
    ...
    print("[3/6] mod 6 잔여 분포 검사...")
    mod6 = mod6_scan(values, grid)
    print("[4/6] 점대칭 쌍 배정 순서 검사...")
    ...
    analysis["verdict"] = (
        ...
        "점대칭 쌍 271)은 주석과 완전히 정합하지만, 값의 배치 순서 규칙은 "
...
def write_reverse_markdown(analysis: dict, path: str) -> None:
    ...
    lines = [
        ...
        f"| 점대칭 쌍 건설적 배정 순서 | 나선 순회 기준 연속 구간 | 없음 — 최장 {s['pair_order']['longest_consecutive_run']}쌍 |",
        f"| 광선 차분 규칙 | 마주 보는 광선 비교 | 부호 반전 — 점대칭 쌍 가설의 자동 귀결 |",
    ...
    lines += [
        ...
        "- 합 조건: 점대칭 쌍 271, 고리 813k, 변 1355, 축 2439, 섹터 6097/6098,",
