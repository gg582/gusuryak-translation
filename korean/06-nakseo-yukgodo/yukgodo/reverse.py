...
def build_reverse_analysis(values: dict, grid: HexGrid,
    ...
    analysis["verdict"] = (
        "현재 복원된 조건은 다수의 해를 허용하며, 서로 다른 최적해 사이에 공통된 국소 생성 지문이 확인되지 않았다. "
        "이는 원래 생성법을 복원하지 못했다는 뜻만이 아니라, 특정 정규 순열이 애초에 도상의 정의 조건이 아니었을 가능성과도 부합한다."
    )
    return analysis
...
def write_reverse_markdown(analysis: dict, path: str) -> None:
    ...
    lines += [
        ...
        "### 미확정 및 반증된 것",
        "",
        "- 검증한 ±6 이동 및 고리별 등차 배정 모형은 모두 실패함.",
