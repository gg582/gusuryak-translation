#!/usr/bin/env python3
"""핵심 불변량 힌트 및 C6 x Z2 대칭군 궤도 연산자를 완전 결합한 최종 해 생성기 (Universal Unified Orbit Solver).

특징:
1. Base Deterministic Backtracking DFS 로 씨앗 해(Base Seed Solution)를 결정론적으로 도출.
2. 핵심 기하 불변량 (대척쌍 271, 고리 813k, 축 2439 등)을 보존하는
...
class UniversalUnifiedOrbitSolver:
    """핵심 불변량 힌트 및 C6 x Z2 대칭 궤도 연산자를 통합한 최종 유니버설 결정론적 생성기."""
    ...
    def verify_all_orbit_invariants(self, family: list[dict[Cell, int]]) -> bool:
        """생성된 궤도 해 12개 전체가 핵심 불변량(페널티 6.0, 고리 813k, 축 2439 등)을 100% 보존하는지 검증."""
        all_valid = True
...
def run_universal_generator_experiment(outdir: str = "yukgodo/experiment") -> dict:
    ...
    print(f"  - 12개 해 전체의 핵심 기하 불변량(페널티 6.0, 고리 813k, 축 2439) 보존 여부: {'100% 통과 (Pass)' if invariants_passed else '실패'}")
    ...
    report = {
        ...
        "conclusions": (
            "1. [최종 결합 솔버의 성능]: 핵심 불변량 힌트와 C6 x Z2 대칭군 궤도 연산자를 완전 결합한 "
