"""mod 5 잔여류 채색 시각화 + 5개 층의 기하 관계 전수조사.

최적해(output/solution.json)의 값 v를 v mod 5로 분류해 채색한다:

    잔여 1 = 검정, 잔여 2 = 빨강, 잔여 3 = 파랑, 잔여 4 = 회색, 잔여 0 = 노랑

270칸은 잔여류별로 정확히 54칸씩 5개 층으로 갈라진다(270 = 5×54).
이 모듈은 5개 층을 낱낱이 분리한 뒤 다음을 전수조사한다:
    ...
    7. 대점(점대칭)의 잔여류 작용: 합 271 ≡ 1 (mod 5) → r ↦ (1−r) mod 5
...
def analyze_mod5(values: dict, grid: HexGrid) -> dict:
    ...
    # 2b. 쌍별 대점 짝지음 행렬 A[i][j] = i층 셀의 대점이 j층에 속하는 칸 수
    ...
    # 7. 대점의 잔여류 작용: 합 271 ≡ 1 (mod 5) ⇒ v의 대점 값 ≡ (1−r) mod 5
    antipode_class = {}
    mismatches = 0
    for c, v in values.items():
...
def draw_mod5_coloring(values: dict, grid: HexGrid, path_png: str,
    ...
    handles = [plt.Line2D([0], [0], marker="o", color="w",
                          ...
                          label=f"잔여 {r} ({CLASS_NAMES[r]}) — 54칸")
               for r in (1, 2, 3, 4, 0)]
    ax.legend(handles=handles, loc="upper right", fontsize=13,
              framealpha=0.95)
    ax.set_title("洛書六觚圖 — mod 5 잔여류 채색 (1 검정·2 빨강·3 파랑·4 회색·0 노랑)",
                 fontsize=18, pad=18)
    ax.autoscale_view()
    fig.tight_layout()
...
def draw_mod5_layers(values: dict, classes: dict, class_sums: dict,
    ...
    for ax, r in zip(axes, (1, 2, 3, 4, 0)):
        ...
        ax.set_title(f"잔여 {r} ({CLASS_NAMES[r]})\n54칸, 층 합 {class_sums[r]}",
                     fontsize=15)
        ax.autoscale_view()
    fig.suptitle("洛書六觚圖 mod 5 — 잔여류별 5개 층 분리", fontsize=20)
...
def draw_mod5_symmetry(values: dict, classes: dict, grid: HexGrid,
                       path_png: str) -> None:
    """합동 쌍 (2,4), (1,0): 한 층과 다른 층의 대점상(회전180°)을 겹쳐 그린다.

    색칠된 원 = 잔여 r1 층, 속 빈 테두리 원 = 잔여 r2 층의 대점상.
    ...
    for ax, (r1, r2) in zip(axes, pairs):
        ...
        ax.set_title(
            f"잔여 {r1} ({CLASS_NAMES[r1]}) ● vs 잔여 {r2} ({CLASS_NAMES[r2]})의 대점상 ○\n"
            f"일치 {match}/54칸 — 회전180° 합동", fontsize=16)
        ax.autoscale_view()
    fig.suptitle("洛書六觚圖 mod 5 — 층 간 점대칭 합동 증명 "
                 "(대점쌍 합 271 ⇒ v의 대점 값 ≡ 1−v mod 5)", fontsize=18)
    fig.tight_layout(rect=(0, 0, 1, 0.94))
    fig.savefig(path_png, dpi=160, bbox_inches="tight")
    plt.close(fig)
...
def write_mod5_markdown(a: dict, path: str) -> None:
    L = []
    L.append("# 洛書六觚圖 mod 5 잔여류 분석\n")
    L.append("값 v를 v mod 5로 분류 (1 검정 · 2 빨강 · 3 파랑 · 4 회색 · 0 노랑).")
    L.append("270칸 = 5류 × 54칸. 층 합: " +
             ", ".join(f"잔여 {r} = {a['class_sums'][r]}" for r in (1, 2, 3, 4, 0)) +
             " (값 공간에서 결정적).\n")

    L.append("## 1. 층 간 D6 합동 전수조사 (240 = 5×4 순서쌍 × 12원소)\n")
    L.append("**합동 쌍: 잔여 2 ↔ 잔여 4, 잔여 1 ↔ 잔여 0 (회전180° = 점대칭).**\n")
    if a["congruences"]:
        L.append("| 층 | → 층 | 대칭원소 |")
        L.append("|---|---|---|")
        for h in a["congruences"]:
            L.append(f"| 잔여 {h['from']} | 잔여 {h['to']} | {h['by']} |")
    else:
        L.append("합동 없음.")
    L.append("")
    ...
    for r1 in range(5):
        for r2 in range(r1 + 1, 5):
            ...
            L.append(f"| 잔여 {r1} ↔ 잔여 {r2} | {best['overlap']}/54 | {best['by']}{mark} |")
    L.append("")
    ...
    L.append("## 3. 대점(점대칭)의 잔여류 작용 — 합 271 ≡ 1 (mod 5)\n")
    L.append("대점쌍 합이 271이므로 잔여류는 r ↦ (1−r) mod 5 로 작용해야 한다.")
    ...
    L.append("")
    L.append("대점 짝지음 행렬 A[i][j] = i층 셀의 대점이 j층에 속하는 칸 수 "
             "(대각선 외 0이면 층이 닫혀 있음):\n")
    ...
    L.append("")
    L.append("해석: A[0][1] = A[1][0] = 54 (0층과 1층이 서로의 대점상),")
    L.append("A[2][4] = A[4][2] = 54 (2층과 4층이 서로의 대점상),")
    L.append("A[3][3] = 54 (3층은 자기 자신의 대점상 — 27개 점대칭 쌍).")
    L.append("이것이 §1의 합동 전부다: 0↔1, 2↔4의 회전180° 합동과 3층의")
    L.append("자기대칭은 대점쌍 합 271의 mod 5 잔여류 작용 r ↦ 1−r 그 자체다.")
    L.append("증명 그림: `mod5_symmetry.png` (색칠 층 vs 상대 층의 대점상 ○ 겹침).")
    L.append("")
    ...
    L.append("|---|---|---|")
    for r in (1, 2, 3, 4, 0):
        L.append(f"| 잔여 {r} | {a['internal_edges'][r]} | {a['components'][r]} |")
    L.append("")
    ...
    for p in a["iso_pairs"]:
        ...
        L.append(f"- 잔여 {r1} ↔ 잔여 {r2}: {verdict} ({note})")
    L.append("")
    ...
    L.append("|" + "---|" * 6)
    for i in range(5):
        L.append(f"| 잔여{i} | " + " | ".join(str(x) for x in a["quotient_matrix"][i]) + " |")
    L.append("")
    ...
    for r in (1, 2, 3, 4, 0):
        u = a["uniformity"][r]
        L.append(f"| 잔여 {r} | {u['wedges_chi2']:.2f} | {u['rays_chi2']:.2f} | {u['rings_chi2']:.2f} |")
    L.append("")
    ...
    L.append("2. 잔여류 위의 작용은 대합 r ↦ S − r 하나뿐이며, 대합의 궤도는")
    ...
    L.append("의의와 한계: 이 성질은 대점쌍 가설을 만족하는 **모든** 해(시드 42")
    ...
    L.append("갖지 않는다면 대점 보수쌍 가설이 기각된다는 의미에서 반증 도구다.")
    L.append("")
...
def main() -> None:
    ...
    print(f"대점 짝지음 행렬: {a['antipode_pairing']}")
    print(f"자기대칭: {a['self_symmetry']}")
    print(f"대점 작용 (불일치 {a['antipode_mismatches']}건): {a['antipode_class_map']}")
