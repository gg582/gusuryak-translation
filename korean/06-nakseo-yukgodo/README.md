...
내적법(來積法) 알고리즘의 친필 주석은 스캔이 매우 흐려 원본 활자를 한눈에 알아보기 어려웠습니다. 이에 따라 초기의 단순 AI 자동 재구 데이터(환각이 섞인 의사전사 데이터)는 전면 폐기하였습니다. **스캔본의 명암·대비(Contrast)를 정밀 조정한 후 뭉개진 모든 글자의 획을 세부 이미지로 분할·분석하는 이미지 정밀 복원 공정**을 거쳐 **친필 주석 원문을 자형 단위로 완전 판독·전사해낸 결과물이 바로 [ALGO_OCR_SUCCESS.md](file:///home/yjlee/gusuryak-translation/korean/06-nakseo-yukgodo/ALGO_OCR_SUCCESS.md)**입니다.

이 수기 전사문에 나타나는 핵심 수치와 계산 관계를 대수적 계산 그래프 분석(`python3 -m yukgodo.naejeok`)으로 검증함으로써, 친필 주석이 숫자의 공간 배치 알고리즘이 아닌 **육각 격자 전체 칸 수(積=271, 虛一 270)를 검산하는 면적 산출 계산 사슬**이라는 고신뢰도 해석을 도출했습니다.

---

# 洛書六觚圖 복원 탐색 프로젝트

주석 판독 수치 조건을 만족하는 배치(최적해)를 탐색으로 역산하는 Python 3 프로그램.
...
2. 添六을 값의 ±6 이동(192변형) 또는 고리별 등차로 본 192개 구체적 건설 모형의 수치적 반증 (`output/hypotheses.json`)
...
* 그러나 탐색 역산(`output/hypotheses.json`)을 통해 添六을 값의 ±6 이동 또는 고리별 등차로 보는 192개 구체적 건설 모형을 수치적으로 반증했습니다.
* 친필 주석은 숫자의 공간 배치가 아니라 **육각 격자의 전체 칸 수(積=271, 虛一則 270)를 구하는 적수 산출 계산 절차(Calculative Algorithm for Hexagonal Grid Area)**라는 해석이 높은 신뢰도로 지지됩니다.
