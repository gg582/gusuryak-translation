...
아래 순위는 "재인쇄가 육고도의 해석을 준비한다"는 조건 아래의 종합 판정이다. 준거는 (i) 직전 전사의 직접성, (ii) `洛書六觚圖`·`通加洛書數六倍` 및 다른 낙서 도상의 구성 선례, (iii) 추가로 선택해야 하는 임의 규칙의 수, (iv) 같은 균형 목표에서의 실험 비용이다.
...
| 2 | **9고리 낙서 제어 + 영역 집계, 별도 배치법** | 직전의 `外合`·`各得五十四`에 가장 직접적으로 맞고, 육고도의 `通加洛書數六倍`도 그대로 보존한다. 다만 "별도 法"이 무엇인지 전사되지 않아 값-위치 대응을 결정하지 못한다. |
