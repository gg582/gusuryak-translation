...
내적법(來積法) 알고리즘의 친필 주석은 스캔이 매우 흐려 원본 활자를 알아보기 어렵습니다. 초기에 생성형 AI 이미지 재구성으로 작성된 원고는 추측성 환각 데이터가 섞여 있어 폐기되었고, 원문 수치 검증과 한문 문헌 대조를 통해 **수기 수동 판독 및 정밀 전사를 완료한 수치 증거 문서가 바로 [ALGO_OCR_SUCCESS.md](file:///home/yjlee/gusuryak-translation/korean/06-nakseo-yukgodo/ALGO_OCR_SUCCESS.md)**입니다.

이 수기 판독 수치들을 대수적 그래프 분석(`python3 -m yukgodo.naejeok`)으로 역산함으로써, 친필 주석이 숫자의 공간 배치 알고리즘이 아닌 **육각 격자 전체 칸 수(積=271, 虛一 270)를 검산하는 면적 산출 계산 사슬**임을 확정지었습니다.
