...
낙서육고도 여백의 **來積法(내적법)** 친필 주석은 스캔본의 흐린 필획을 정밀 분석하여 **자형 단위로 완전 판독·전사**를 마쳤습니다([ALGO_OCR_SUCCESS.md](file:///home/yjlee/gusuryak-translation/korean/06-nakseo-yukgodo/ALGO_OCR_SUCCESS.md)). 초기 생성형 AI가 출력한 `五百六`(506) 오독 및 잘못된 줄바꿈 등 환각이 섞인 의사(擬似)전사 데이터는 전면 폐기하였으며, 수동 자형 재판독과 대수적 계산 그래프 교차 검증을 결합하여 확정 전사문을 완성했습니다.
...
九乘得二百五 midday(二百五十二)  (10+18) × 9 = 252     = 2 × 126
...
├── reverse.py      # 후보 생성 규칙 및 국소 지문의 검증 + 주석 대조
...
**현재 채택한 가설 아래 구성한 최적 증인해(페널티 6.0)를 산출했다.** 검증된 성질:
...
이 배치는 유일한 원본 도안을 직접 복원한 것이 아니라, **현재 채택한 가설 하에서 구한 최적 증인해**다.

## 가설 검증 결론 (`output/hypotheses.json`)

1. 주석에서 판독 가능한 수치는 전부 **기하 골격 및 칸 수 검산**이다 — 54+6=60, 60/6=10(변당 칸), 中觚 19, 252, 252×2=504, 270=6×45. 즉 주석의 검증 가능한 내용은 도안의 기하 구조와 그에 따른 적수 수리 체계이며, 이는 전부 확인되었다.
2. 검증한 ±6 이동 및 고리별 등차 배정 모형은 모두 실패했다. 적어도 현재 제안된 구체적 값 배치 독해들은 성립하지 않는다.
3. `寄左`·`序左`의 자형 판독은 완료되었으나, 이들이 중간값 보관·계산 진행·도형 전개 중 어느 기능을 가리키는지는 산학 문맥상 미확정이다. 현재로서는 특정 셀의 정규 배정 순서를 나타낸다는 증거가 없다.
...
```bash
python3 -m yukgodo.reverse    # 검증 → output/reverse_engineering.{json,md}
```
...
- **미해결 (산법 기능 미확정)**: `寄左`·`序左`·`以算遠則係以六` 등의 자형 판독은 완료되었으나, 이들이 어떤 산학적 기능을 가리키는지는 문맥상 미확정이다.

**생성 규칙 존재 여부의 탐색적 검토:**

1. 다른 시드(42)로 찾은 최적해는 기존 최적해와 **0/270칸**만 일치한다. 즉 합
   조건을 만족하는 배치가 다수 존재하며, 현재 복원된 조건은 다수의 해를 허용한다.
2. 서로 다른 최적해 사이에 공통된 국소 생성 지문이 확인되지 않는다. 이는 원래 생성법을 복원하지 못했다는 뜻만이 아니라, 특정 정규 순열이 애초에 도상의 정의 조건이 아니었을 가능성과도 부합한다.��이프라인 엔트리포인트
tests/test_hexgrid.py
output/             # solution.json, nakseo_yukgodo.png/.svg, dashboard.png, report.md,
                    # siamese_report.md, reverse_engineering.md, mod5_*.png/.json/.md 등
```
