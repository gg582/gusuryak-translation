...
이 해석에는 사용자가 직접 OCR한 병편의 [병행 원문 OCR](BING_PARALLEL_OCR.md)이 추가 내부 근거를 제공합니다. `每十八隻包中六外成六觚`와 이어지는 `添六隻`·`以十二而一得八隻`은 `包`·`隻`·`添六`·`而一`을 육각 형성 및 적수 계산의 한 어휘권에 놓습니다. 병편 원문은 자동 보정 없이 보존하고, 작업용 띄어쓰기·단락본은 원문과 구분해 기록했습니다. 이 `包` 독해 — 중심 노드를 여섯이 원형으로 감싼다 — 는 《구수략》 곤(坤)책 간행 번역(정해남·허민 역, 교우사) 11쪽의 `六包一`에서 독립적으로 확인됩니다. 그곳에서는 1+(6+12+18+…) 형태의 급수를 구하는 문맥이며, 이는 육고도의 고리 산법(1 + 6×45 = 271, 虛一 뒤 270)과 항 대 항으로 맞물립니다. 같은 판본 48쪽의 원전 양식은 [minimal.md](minimal.md)에 별도 전사했습니다. 이로써 내적법의 기하·적수 독해는 강화되지만, 대척 셀의 값합 271을 직접 지시하는 문구가 새로 생기는 것은 아닙니다.

## 경쟁 배치 조건과 직전 천수용오도 재인쇄

대척보수만을 전제하지 않고 비대척 보수쌍, 고리 내부 보수쌍, 6회전 궤도합 813, 반대 영역 집계만, 9고리 낙서 제어+별도 배치, 45칸 한 隻의 6회전 생성을 모두 구현해 같은 기하·값집합·균형 목표에서 비교했다. 상세한 조건, 재현 명령, 첫 실행값은 [ALTERNATIVE_CONDITIONS_AUDIT.md](ALTERNATIVE_CONDITIONS_AUDIT.md)에 둔다.

이 비교에서 육고도 바로 앞에 재인쇄된 천수용오도 계열의 전사

> `去中宮十五` · `外合得二百一十六` · `無宮各得五十四爲六九之數`

를 의도적 선행 준거로 읽으면, `216=4×54`와 `54=6×9`가 직접 나온다. 따라서 재인쇄가 가장 강하게 지지하는 것은 중심 제외 뒤 외부 영역의 **등집계**와 `54=六九`의 수적 연결이다. 그것은 셀별 대척보수의 직접 명령은 아니며, 각 경쟁 가설을 판정하는 별도의 준거다.

### 조건부 개연성 순위

아래 순위는 "재인쇄가 육고도의 해석을 준비한다"는 조건 아래의 종합 판정이다. 준거는 (i) 직전 전사의 직접성, (ii) `洛書六觚圖`·`逓加洛書數六倍` 및 다른 낙서 도상의 구성 선례, (iii) 추가로 선택해야 하는 임의 규칙의 수, (iv) 같은 균형 목표에서의 실험 비용이다.

| 순위 | 조건군 | 판정 근거 |
| ---: | --- | --- |
| 1 | **문헌 내적 대척보수 복원원리** | 낙서 계열의 위치-값 보수 구성 선례, 135 위치 대척쌍과 135 값 보수쌍의 정확한 대응, 고리 `6k↔6(10-k)`의 합 60을 함께 보존한다. 직전 재인쇄의 `去中宮`·`54=6×9`와도 양립하며, 균형 편차 6·고리/축 편차 0이다. 직접 셀별 명령은 아직 없다. |
| 2 | **9고리 낙서 제어 + 영역 집계, 별도 배치법** | 직전의 `外合`·`各得五十四`에 가장 직접적으로 맞고, 육고도의 `逓加洛書數六倍`도 그대로 보존한다. 다만 "별도 法"이 무엇인지 전사되지 않아 값-위치 대응을 결정하지 못한다. |
| 3 | **고리 내부 비대척 보수쌍** | 각 고리 안에서 보수쌍을 만들면 `813k`가 자동으로 나온다. 실험 편차도 10으로 낮다. 그러나 이웃 등 어떤 쌍을 고를지 원문 준거가 없다. |
| 4 | **반대 영역의 집계합만 일정** | 직전 재인쇄와 가장 닮은 집계 수법이며, 실제로 편차 6의 비대척 반례가 존재한다. 바로 그 때문에 셀별 배정법으로는 너무 약하다. |
| 5 | **6회전 궤도합 813** | `6k` 고리 기하와 잘 맞고 고리합을 자동 보존한다. 그러나 `六轉`·궤도합 813의 원문 지시가 없고, 현재 균형 편차는 127이다. |
| 6 | **45칸 한 隻 + 고정 6순환 값변환** | 45개의 6회전 궤도를 하나의 값변환으로 생성하는 명료한 현대 모형이다. 하지만 가장 강한 추가 가정이며, 직전 재인쇄에도 회전·변환 지시가 없고 편차 300이다. |
| 7 | **임의의 전역 비대척 보수 매칭** | 수학적으로는 편차 7까지 가능하지만, 어떤 위치를 짝지을지 원문 준거가 전혀 없어 선택이 가장 임의적이다. |

따라서 천수용오도 재인쇄는 대척보수를 밀어내는 반증이 아니라, **고리 제어·중심 제외·영역 집계**라는 독립 층을 강하게 보여 준다. 이 층을 모두 남겨도 값 배치를 가장 적게 임의로 선택하는 안은 1위의 대척보수 복원원리이고, 집계만으로 그 원리를 논리적으로 강제하지 못한다는 한계는 2위·4위 모델이 동시에 보여 준다.
...
**대척보수(대척 셀 값의 합 271)는 이 프로젝트에서 가장 흥미로운 가설 중 하나지만, 확정된 사실이 아니다.** 원문 어디에서도 이를 직접 명령하는 문구가 전사되지 않았고, 반례 감사는 나머지 집계 조건이 그것을 논리적으로 강제하지 않음을 보였다. 이후의 모든 탐색 결과는 이 가설을 채택했을 때의 조건부 귀결이며, 원문이 실제로 그렇게 배치했다는 직접 증거는 아니다.
...
이 원리의 가장 이른 책 내부 출발점은 `數原第一`의 하도·낙서 설명이다. `併上下數共一十一`과 `化裁五格各得一十一數`은 양 끝 수를 보수쌍으로 묶어 일정합을 만드는 계산을 직접 보이고, 이어지는 낙서 3×3은 대척 위치에서 `4+6=9+1=2+8=3+7=10`, 중심 5 고정을 실제로 배치한다. `河圖為體`·`洛書為用`의 연속 뒤 `洛書六觚圖`·`逓加洛書數六倍`와 `54+6=60`이 나오는 사슬은, 육고도 대척보수 복원원리의 문헌 내적 근거를 한층 강화한다. 원문 인용과 범위는 [HETU_LUOSHU_ORIGIN_EVIDENCE.md](HETU_LUOSHU_ORIGIN_EVIDENCE.md)에 정리한다.
...
3. `逓加洛書數六倍`는 낙서의 `k↔10-k`를 `6k↔6(10-k)`로 올리고, 來積法은 그 첫 쌍을 정확히 `54+6=60`으로 계산한다.
...
- 총 칸 수 270 = 6×(1+2+...+9) = **6×45** (逓加洛書數六倍)
- 중앙 가로줄(中觚) **19칸** (十九爲中觚數也)

## 재구 가설: 최석정의 다른 도상에서 확인되는 보수대합 수법의 적용

- 값 1..270을 한 번씩 배치 (지수귀문도 1..30, 후책용구도 1..72 등과 같은 체계)
- 대척점(점대칭) 위치의 두 값은 합 **271**의 보수쌍 (重象用九圖 등의 보수쌍 수법)

이 가설 하에서 고리 k 합 = 813k, 중심 공백을 제외한 각 대척축의 18개 값 합 = 9×271 = 2439는 구조적으로 자동 성립하고,
탐색은 변·섹터·광선의 균형만 찾는다.

## 재구 가설: 고리별 연속값 배치 (Z3 산술 복원)

`z3_arithmetic_reconstruction.py`는 또 하나의 유력한 가설을 형식화한다. 원문에서 직접 지지되는 산술 구조 — 逓加의 고리 크기 6k, 來積法의 사슬(54+6=60, 60÷6=10, 2×10−1=19, (10+18)×9=252, 252+19=271), 虛一 270 — 를 Z3 제약으로 부호화하고, 개별 값 복원을 위해 다음 가설을 얹는다.

- 값 1..270을 한 번씩 배치
- 고리 k는 연속 구간 3k(k−1)+1 … 3k(k+1)을 받는다 (고리 1: 1..6, …, 고리 9: 217..270)
- 각 고리 안에서 값은 원주를 따라 연속적으로 진행한다

고리의 시작점과 회전 방향은 회전 대칭의 정규화일 뿐 원문 지시로 주장하지 않는다. 이 가설은 대척보수 가설과는 별개의 후보다(고리합은 813k가 아니라 18k³+3k가 된다). 실행하면 제약 만족을 검증하고 배치도(`reconstruction_1.png`)를 그린다. 솔버와 결과(output/)는 독립 미니 리포 [yukgodo2/](yukgodo2/)에 분리해 두었다.
...
python3 z3_arithmetic_reconstruction.py  # 고리별 연속값 가설의 Z3 산술 복원 + 배치도
```
...
이 배치는 來積法의 절차나 유일한 원본 배열을 재현한 것이 아니라, 문헌에서 복원한 격자 사양에 문헌 내적 대척보수 복원원리와 현대적 균형 목적함수를 적용해 산출한 조건부 최적 증인해다. 특히 고리합·축합은 이 복원원리의 자동 귀결이며, 원문이 독립적으로 지시한 값 배치 조건으로 역전하지 않는다. 다시 말해 대척보수는 흥미로운 가설이자 유력한 후보 해석 중 하나일 뿐 확정된 원문 조건이 아니며, 역사적 도안이 실제로 이를 따랐는지는 미해결로 남는다.
...
- **확정 (칸 수·기하)**: 共積二百七十, 虛一則二百七十數, 校計周五十四數, 逓加洛書數六倍(270=6×45), 十九爲中觚數也, 置外周五十四以九乘之得四百八十六 및 折半加九得二百五十二(외주 54 * 9 / 2 + 9 = 252 기하학적 유도 공식의 확정), 倍之得五百四 및 折半得二百五十二(504 배배 및 절반 검산 공식의 확정), 合從九目得二百五十二 및 去中觚(9개 고리 270칸에서 중고 18칸 제외 시 252), 置外周添六(고리가 6칸씩 늘어남이라는 칸 수 독해).
