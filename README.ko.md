...
- 10차 백자 계통 배열은 원문 전사 상태로 보존·분석하며, 마방진에 맞추기 위한 정규화는 하지 않습니다. 별도로 기록한 백자도 재배열은 행·열의 합만 `505`인 준마방진입니다.
- **구구변운양도**는 九宫(구궁) 프레임워크를 기반으로 한 추가 변환 도상입니다.
- 6개 도상 모두 pan-diagonal은 아닙니다.

백자 계통의 원 배열은 전사 자료로 보존하고, 분석에서는 실제로 기록된 값과
불변량을 다룹니다. 마방진 조건에 맞춘 1..100 정규화본은 만들지 않습니다.
별도로 문서화한 백자도 재배열은 행·열 합이 505인 준마방진입니다.
