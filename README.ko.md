...
> **읽기 안내.** 이 저장소는 원문 직접 근거, 전사, 구조 가설, 수학적 귀결, 계산 실험을 구분합니다. 재구성이나 솔버 출력을 역사적 주장으로 읽기 전에 [EVIDENCE.ko.md](EVIDENCE.ko.md)를 참고하십시오.

《구수략(九數略)》 등 고전 문헌에 전하는 한중 수학 도상들을 현대 조합론적으로 재정의하고, 검증 코드와 보고서 및 시각화 자료를 모아둔 저장소입니다.

이 프로젝트의 중심은 완전한 기원·전파사가 아니라 전산수학적 번역입니다.
핵심 질문은 약 300년 전의 학자들도 등합, 수의 전부 사용, 대칭, 건설, 검산과
비슷한 문제를 다루었는가입니다. 우선권·직접 전파·저자의 의도에 관한 주장은
현존 자료가 입증하는 범위와 전산적 해석을 분리해 기록합니다.
...
- 낙서사구도는 20개 노드의 이분 그래프로, 외곽 해밀턴 20-cycle과 합 42인 내곽 4-cycle을 가집니다.
- 낙서오구도는 `1`부터 `33`까지를 한 번씩 9개 십자형 궁에 배치합니다. 각 궁의 합은 `85`이고, 중복 가중 총합은 `765`입니다.
- 낙서칠구도는 `1`부터 `63`까지를 3×3 낙서 궁격에 배치합니다. 각 궁은 7개 수를 받아 원문 제약상 합 `224`(= 7 × 32)를 이룹니다. OCR 충돌 칸은 디렉토리의 MILP solver로 재구성합니다.
- 낙서팔구도는 `1`부터 `80`까지를 5개의 8×4 블록 구조에 배치하여 각 궁/클러스터의 합이 `164`가 되도록 합니다.

#### 낙서칠구도의 두 가지 읽기

**각득 해석.** 이 도상은 `1`부터 `63`까지의 정수를 9개의 7원소 블록으로 분할한 것이며, 각 블록의 합은 `224`(= 7 × 32, 1..63의 평균)입니다. 사구도·오구도와 달리 중복이 없으므로 `作 = 用 = 63`이고 중복 가중 합 `D = 0`입니다. 현재 OCR 입력은 몇몇 칸에서 원문 제약과 충돌하므로, MILP 재구성값을 기준으로 합니다.

| 궁 (중심) | 7개 수: 중심 먼저, 그 뒤 12시부터 시계방향 | 합 |
|---|---|---:|
...
- 4개 `10×10` 원 배열 중 3개는 정상 수 집합 검사를 통과하지 못하고, 나머지 1개는 정상 집합이지만 마방진은 아닙니다. `1..100`을 한 번씩 사용하고 모든 행·열·대각선의 합이 `505`인 교정안은 각 `corrected.md`에 제공되어 있습니다.
- **구구변운양도**는 九宫(구궁) 프레임워크를 기반으로 한 추가 변환 도상입니다.
- 6개 도상 모두 pan-diagonal은 아닙니다.

백자 계통의 원 배열은 전사 자료로 보존하되, 정상 집합이 아닌 배열은 정상적인
`1..100` 집합이었다는 기준으로 교정합니다. 각 `corrected.md`에는 실제 검산을
통과한 10×10 교정 배열을 싣습니다. 네 교정 배열은 모두 1부터 100까지를 한 번씩
쓰고, 모든 행·열·대각선의 합이 505입니다. 인쇄본 간 차이 또는 출판·검수 과정의
필사 오류가 원 배열의 손상을 낳았을 가능성도 각 분석 문서에 기록했습니다.

### 05. 추가 다섯 도안

| 도안 | 영문 경로 | 한국어 경로 |
|---|---|---|
| 기책용팔도 | `english/05-extra-five/gichaek-yongpaldo/` | `korean/05-extra-five/기책용팔도/` |
| 범수용오도 | `english/05-extra-five/beomsu-yong-odo/` | `korean/05-extra-five/범수용오도/` |
| 장책용칠도 | `english/05-extra-five/jangchaek-yong-chil-do/` | `korean/05-extra-five/장책용칠도/` |
| 중상용구도(象上用九圖) | `english/05-extra-five/jungsang-yong-gudo/` | `korean/05-extra-five/중상용구도/` |
| 중의용육도 | `english/05-extra-five/jungui-yong-yukdo/` | `korean/05-extra-five/중의용육도/` |

고정된 원문 도안의 합·중복·자리 수 검증은 다항 시간(P)입니다. 임의 가중치·서로
겹치지 않는 그룹·목표합을 허용하는 일반 등합 배치 문제는
`COMPLEXITY.ko.md`의 3-PARTITION 환원으로 증명됩니다. 고정된 역사 도안과
연속 수열 변형은 이 환원만으로 NP-완전이라 할 수 없습니다.

각 도안 디렉터리에는 01~04 계열과 같은 심층 성질 분석(`analyze_*.py` → 그림
`01`~`08`, `analysis_report.md`, 클러스터별 `analyze_rotations.py` →
`rotation_report.txt` / `rotation_cluster_*.png` / `rotation_overview.png`)이
들어 있습니다. 주요 발견:

- **기책용팔도**(4개 팔각형 합 100, 값 1..24, D=100): 공유 꼭짓점 차수 3,
  중앙 정사각형과 두 공유 4점 합 50 = S/2, 팔각형별 공유/고유 50/50,
  고리 합 50·50·100·100, 이분 스펙트럼 λ_max = φ².
- **범수용오도**(2축 합 25, 값 1..9, D=5): 트리 S(2,2,2,2), 네 팔 합 모두 10,
  고리 합 20/20, 가로축이 상생 순환, λ_max = √5.
- **장책용칠도**(3축 합 68, 값 1..19, D=14): 거미형 트리, 고리 합 61×3,
  스포크 합이 연속 6정수 28~33, 3×3 반대편 쌍 합 행렬이 준마방진(행·열 합 61).
- **중상용구도**(4축 합 147, 값 1..33, D=27): 8개 광선 합 모두 69
  (축 147 = 69+9+69), 고리 합 138×4(고리+중심 = 147로 원문 「周圍四重各得147」와
  정합), 원문의 「演積 1148」은 자연 산식(1140/1149/1176)과 어긋난 미해결
  불일치로 기록.
- **중의용육도**(4개 겹침 그룹 합 51, 값 1..16, D=68): 공유 8값 = 비공유 8값 = 68,
  상·하단 그룹만 합 17 보수 쌍 3개로 분해(좌·우측은 불가), 행 합 회문.
- 세 별형 도상(범수 2×5, 장책 3×7, 중상 4×9)은 N = a(L−1)+1, 중심 = L,
  축 합 S = (T+(a−1)L)/a, 고리 합 R = (T−L)/a, 같은 방사선 합 불변량이라는
  관찰된 가족 패턴을 이룹니다.
...
| 도안 | 영문 경로 | 한국어 경로 |
|---|---|---|
| 낙서육고도(洛書六觚圖) | `english/06-nakseo-yukgodo/` | `korean/06-nakseo-yukgodo/` |

270칸 해, 원문 주석 분석, 기하 불변량 테스트, JSON 지표, SVG/PNG 도안,
Siamese식 지역 규칙 검토와 함께, 최종 재구 도안에서 출발해 후보 생성 규칙을
검사하고 흐린 주석과 대조하는 생성 규칙 역산(`yukgodo/reverse.py` →
`output/reverse_engineering.md`)이 들어 있습니다. 판정: 기하 골격과 합 조건은
주석과 정합하지만, 최적해가 다수 존재하고(시드 간 일치 0/270칸) 건설적 흔적이
남지 않아 배치 알고리즘 자체는 현재 증거로 확정할 수 없습니다 — 내적법 계산 사슬은 새 OCR 판독으로 본체 사슬 1급 확정에 도달했으나, 배치 순서 지시(序左)와 꼬리 부분(得五百)의 확정에는 더 선명한 판본이 필요합니다. 주어진 배치의 검증은 P이고, 임의 반지름에서
보수쌍·고리·변·섹터·광선 제약을 모두 만족하는 배치의 존재 문제는 현재 완전한
위상 보존 환원이 없으므로 미증명으로 둡니다. 주어진 배치의 검증만 P로 확정합니다.

### 07. 추가 두 도안

| 도안 | 영문 경로 | 한국어 경로 |
|---|---|---|
| 중괘용팔도·팔진도 | `english/07-extra-two/junggwae-yong-paldo/` | `korean/07-extra-two/중괘용팔도/` |
| 후책용구도 | `english/07-extra-two/huchaek-yong-gudo/` | `korean/07-extra-two/후책용구도/` |

고정 전사본과 팔진도 정정본은 합·중복·누락·`1..64` 범위를 직접 검증하며,
이 검증은 P입니다. 임의 개수의 진형과 목표 합을 갖는 일반 배치 존재 문제는
임의 가중치·서로 겹치지 않는 그룹이라는 하위 문제만 3-PARTITION 환원으로
증명됩니다. 정확한 진형 위상을 보존한 존재 문제는 추가 환원 없이는 분류하지
않습니다.
...
| 중괘용팔도(中卦用八圖) | `english/07-extra-two/junggwae-yong-paldo/` | `korean/07-extra-two/중괘용팔도/` |
| 후책용구도(後策用九圖) | `english/07-extra-two/huchaek-yong-gudo/` | `korean/07-extra-two/후책용구도/` |
...
이 프로젝트에서는 각득 해석이 전산적으로 더 유용합니다. 관찰 가능한
불변량을 분리하고 매개변수화된 퍼즐족으로 만들 수 있기 때문입니다. 사오도
해석은 원문의 상징·방위 언어를 보존하는 역할을 합니다. 이는 분석 목적의
차이지 근세 수학의 우열 판단이 아닙니다.
...
역사적 도상 자체는 작고 구체적이라 검증은 모두 다항 시간입니다. 아래 표는
증명된 환원과 미증명인 일반화를 구분합니다. 자세한 환원은
[`COMPLEXITY.ko.md`](COMPLEXITY.ko.md)에, 실제 Python 3 검증 실행은
`python3 complexity/verify_polynomial_checks.py`에 기록되어 있습니다.

| 퍼즐 / 계열 | 부분 문제 | 복잡도 | 이유 / 비고 |
|---|---|---|---|
| 하도/사오도 5-컬러링 | 체크섬 210과 mod-5 색 클래스 검증 | **P** | 직접 합산과 잉여 검사 |
| 하도/사오도 5-컬러링 | 주어진 기하 슬롯 그래프 라벨링이 제약을 만족하는지 결정 | **조건부 / 이 저장소에서 미증명** | k ≥ 3 색칠 및 exact-cover형 슬롯 제약을 포착 |
| 하도/사오도 5-컬러링 | 대합 `σ, τ`와 블록 디자인 교차 행렬 검증 | **P** | 유한하고 상수 크기 |
| 낙서사구도(사자각득) | 모든 궁 합이 42인지 검증 | **P** | 9궁 × 4수 |
| 낙서사구도 | `1..20`을 9개 겹치는 4집합에 합 42로 배치 가능한지 결정 | **조건부 / 이 저장소에서 미증명** | `04-unification`의 MILP 솔버가 작은 경우 처리 |
| 낙서오구도(오자각득) | 모든 궁 합 85, 9궁 총합 765 검증 | **P** | 직접 합산 |
| 낙서오구도 | `1..33`을 9개 십자 궁에 합 85로 배치 가능한지 결정 | **조건부 / 이 저장소에서 미증명** | 사구도와 동일한 겹침 그래프 구조 |
| 낙서칠구도(칠자각득) | 모든 궁 합 224 검증 | **P** | 9궁 × 7수 |
| 낙서칠구도 | `1..63`을 9개 7원소 블록으로 합 224로 분할(낙서 중심 고정) 가능한지 결정 | **조건부 / 이 저장소에서 미증명** | 고정 원소를 가진 등분할 |
| 칠자각득 | 5개 클러스터 각 7수 합 120 검증 | **P** | O(35) 덧셈 |
| 칠자각득 | `Π(5, 6, 120)` 배치 존재 여부 | **조건부 / 이 저장소에서 미증명** | mod-5 중심 제약을 가진 다중 분할 |
...
| 마방진 | 부분 채워진 마방진 완성 | **조건부 / 이 저장소에서 미증명** | 라틴 방진 완성과의 정확한 변환을 이 저장소에서 제시하지 않음 |
| `Π(p, q, T)` / 각득 MILP | 임의 가중치·서로 겹치지 않는 그룹 배치 존재 여부 | **증명된 NP-완전** | 3-PARTITION 항등 환원. 제한된 `(p, q, T)` 변형은 미증명 |
...
2. **존재/완성**: "이 조건을 만족하는 도상이 존재하는가?" — 입력 모델에 따라
달라지며, 이 저장소는 환원을 제시한 경우에만 난도를 주장합니다.
...
6. **별 가족의 존재 조건**: 임의의 `(a, L)`에 대해 등축·등륜·등광선 별형 배치(값 `1..N`, 중심 `L`)가 존재하는가? 중심이 반드시 `L`이어야 하는가?
7. **기책용팔도 배치의 유일성**: 공유/고유 50/50 분할을 만족하는 배치가 대칭을 제외하고 유일한가?
8. **중의용육도의 완전 대칭 배치**: 4개 그룹 모두가 합 17 보수 쌍으로 분해되는 배치가 존재하는가(MILP로 결정 가능)?
9. **중상용구도의 연적 1148**: 어떤 회계 방식이 원문 수치를 산출하는가, 아니면 전사 손상인가?
10. **낙서육고도의 배치 순서 규칙**: 합 조건 최적해가 다수 존재하고 건설적 흔적이 남지 않는 상황에서, 더 선명한 판본이 나오면 내적법의 순서 규칙(寄左/序左)을 복구할 수 있는가?
