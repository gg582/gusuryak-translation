...
고정된 원문 도안의 합·중복·자리 수 검증은 다항 시간(P)입니다. 임의 개수의
궁과 수를 갖는 동등합 배치의 존재 문제는 exact cover 및 정수 분할을 포함하므로
일반화하면 NP-complete입니다.
