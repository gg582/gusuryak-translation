...
# -*- coding: utf-8 -*-
"""
Generalization analyzer for the Gakdeuk (各得) principle

Computational verification script for the shared properties and
generalization models of the Gakdeuk puzzle family from the *Gusuryak (九數略)*
tradition (Gujagakdeuk, Ojagakdeuk, Yukjagakdeuk, Chiljagakdeuk, Paljagakdeuk).
"""
...
class GakdeukFamily:
    """Data class holding the data for one Gakdeuk family."""

    name: str
    n: int  # number of vertices per subset
    k: int  # number of subsets (always 5)
    m: int  # size of the complete Wuxing group (basis for Wuxing sum progression)
...
WUXING_NAMES: Final[tuple[str, ...]] = ("Water", "Fire", "Wood", "Metal", "Earth")


def wuxing_of(n: int) -> str:
    """Classify a number into a Wuxing element (1->Water, 2->Fire, 3->Wood, 4->Metal, 0->Earth)."""
    r = n % 5
...
def wuxing_sum_sequence(m: int) -> list[int]:
    """
    Return the Wuxing-group sum sequence when the numbers 1 through 5*m
...
def print_summary_table() -> None:
    """Print a summary table of the Gakdeuk families."""
    print("=" * 100)
    print("Summary of the Gakdeuk (各得) Families")
    print("=" * 100)
    header = (
        f"{'Family':<28} | {'n':>3} | {'k':>3} | "
        f"{'Range':>8} | {'T':>5} | {'S':>5} | "
...
def print_wuxing_table() -> None:
    """Print the Wuxing-sum arithmetic progression for each family."""
    print("=" * 80)
    print("Wuxing (Five Elements) mod 5 Sum Distribution")
    print("=" * 80)
    print(f"{'Family':<28} | {'m':>3} | Water | Fire | Wood | Metal | Earth | Diff")
    print("-" * 80)
    for fam in FAMILIES:
        # Based on full 5×m range (Ojagakdeuk is extended to 25)
        m = fam.m
...
def print_equation_check() -> None:
    ...
    for fam in FAMILIES:
        if fam.subset_sum is None:
            print(f"{fam.name:<28}: S not specified (Ojagakdeuk)")
            continue
...
def generate_markdown_report() -> str:
    ...
    lines: list[str] = []
    lines.append("# Generalization of the Gakdeuk Principle: Computational Verification Report\n")
    lines.append("This report was automatically generated by `generalize_gakdeuk.py`.\n")

    lines.append("## 1. Summary of the Gakdeuk Families\n")
    ...
    lines.append("## 2. Wuxing mod 5 Sum Distribution\n")
