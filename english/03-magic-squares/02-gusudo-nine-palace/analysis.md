# Gusudo (Nine Palace, 九數圖)

Order: 9×9

9×9 magic square. The magic constant is 9×(81+1)/2 = 369. Both examples are normal magic squares that use each of the numbers 1 through 81 exactly once. The second example is likely in the same magic square group as the first, obtained by row/column permutations or reversals.

## Example 1
- Value range: 1 ~ 81
- Normal set (consecutive integers): Yes
- Reference magic constant: 369
- Row sums: [369, 369, 369, 369, 369, 369, 369, 369, 369]
- Column sums: [369, 369, 369, 369, 369, 369, 369, 369, 369]
- Diagonal sums: [369, 369]
- Semi-magic: Yes
- Magic square: Yes
- Pan-diagonal: No
- Wrapped diagonal sums: [369, 621, 144, 396, 621, 117, 342, 594, 117, 285, 444, 360, 285, 453, 378, 294, 453, 369]
- Associated: Yes (center-symmetric sum = 82)
- Bimagic: No
- 180° rotational symmetry: No

## Example 2
- Value range: 1 ~ 81
- Normal set (consecutive integers): Yes
- Reference magic constant: 369
- Row sums: [369, 369, 369, 369, 369, 369, 369, 369, 369]
- Column sums: [369, 369, 369, 369, 369, 369, 369, 369, 369]
- Diagonal sums: [369, 369]
- Semi-magic: Yes
- Magic square: Yes
- Pan-diagonal: No
- Wrapped diagonal sums: [369, 380, 311, 519, 252, 438, 219, 523, 310, 392, 335, 399, 472, 298, 339, 339, 378, 369]
- Associated: No (center-symmetric sum = 90)
- Bimagic: No
- 180° rotational symmetry: No

## Value Frequency Analysis (Example 1)
- Total cells: 81
- Distinct values: 81
- No duplicates
