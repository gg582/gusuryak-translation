...
# -*- coding: utf-8 -*-
"""
Baekjajasuyin-yang-chakjong (百子子數陰陽錯綜圖) Combined Latin Square Benchmark.

Derives a 10x10 magic square (magic constant 505) via decimal composition of
child (1~10) and parent (0~9) Latin squares, removes isomorphic solutions under
rotations/reflections (D8 symmetry), and performs graph topology analysis.
"""
...
# Base 10x10 magic square obtained from corrected Latin square decimal composition
# M[i, j] = 10 * P[i, j] + C[i, j] where P is in 0..9 and C is in 1..10
...
def generate_circulant_latin_composition() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Generate Parent (0~9) and Child (1~10) circulant Latin squares and their 10x10 composition."""
...
def verify_latin_composition(P: np.ndarray, C: np.ndarray) -> bool:
    """Verify row and column sums for Parent (45) and Child (55) matrices."""
...
def filter_isomorphic_solutions(matrices: list[np.ndarray]) -> int:
    """Remove D8 isomorphic solutions (8 rotational/reflection transformations)."""
...
def plot_latin_visualization(M: np.ndarray, P: np.ndarray, C: np.ndarray, output_path: str):
    """Plot 10x10 magic square matrix and parent/child Latin composition."""
...
def main():
    ...
    # D8 Isomorphism filtering across generated solutions
    non_isomorphic_count = filter_isomorphic_solutions([M, CORRECTED_MAGIC_SQUARE])


    # Graph Topology & Spectral Radius
    ...
    print()
    print("--- Composition & Latin Square Verification ---")
    print(f"Magic Constant Sum (505): Verified for all rows and columns.")
    print(f"Parent (0~9, sum 45) & Child (1~10, sum 55) Latin Composition: {latin_valid}")
