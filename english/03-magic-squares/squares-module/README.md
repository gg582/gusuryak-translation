...
This document is a detailed guide to the mathematical definitions, structural interrelationships, and complementary derivation principles among the corrected versions of the 10×10 Baekjado (百子圖, Hundred-Numbers Diagram) family of magic squares recorded in 《Gusuryak (九數略)》, written by the Joseon-era Silhak scholar Choi Seok-jeong.
...
*   **Mother Square (모도 母圖)**
    ...
    *   **10×10 mapping**: The **corrected version of the Baekjayin-yang-jamo-chakjong (百子陰陽子母錯綜圖, Hundred-Numbers Yin-Yang Mother-Child Intertwining Diagram)** in the `06-baekjayin-yang-jamo-chakjong` folder corresponds to the Mother Square.
*   **Son Square (자도 子圖)**
    ...
    *   **Note**: In the original text of 《Gusuryak》, the `Baekjajasuyin-yang-chakjong` and the `Baekjamosuyin-yang-chakjong` are recorded separately as two individual 10×10 Latin squares (1–10 and 0–9); a single combined 1–100 magic square merging the two is not included in the book. Accordingly, the `03-baekjajasuyin-yang-chakjong` folder contains no corrected magic square (`corrected.md`), only an analysis of the underlying original Latin squares. However, the 1–100 reference square $M_{03}$, defined to explain the algebraic/spatial derivation relationships, is referred to for convenience as the 'Son Square (자도)' and is used only inside this analysis module.
*   **Yang Square (양도 陽圖)**
...
## 2. The Baekjado Generation System and the Principle of Using Corrected Versions
...
### Why Corrected Versions Are Used
The original arrays in the surviving transcription of 《Gusuryak》 have accumulated omissions and duplication errors in some values over centuries of hand-copying, so they fail to satisfy the normal magic-square condition (row/column/diagonal sums of 505).
Therefore, to clarify the arithmetic verification and the analysis of the derivation structure, we use as our reference a **Corrected Version — a complete magic square in which the errors have been fixed and the symmetry adjusted so that every row, column, main diagonal, and anti-diagonal sums to 505**.

---

## 3. Mathematical Derivation Relationships Among the Corrected Squares

The 10×10 Baekjado corrected versions are not individual magic squares made arbitrarily; they are derived organically according to the transformation laws of the two-dimensional spatial symmetry group (Dihedral Group), taking the **reference square ($M_{03}$)** as the reference axis.
...
*   **Derived square**: The corrected Baekjayin-yang-jamo-chakjong of `06-baekjayin-yang-jamo-chakjong` ($M_{06}$, Mother Square)
*   **Derivation formula**:
...
*   **Source square**: The 1–100 reference square ($M_{03}$) defined for the underlying transformation
*   **Derived square**: The corrected Baekjasaengseong-sunsu of `04-baekjasaengseong-sunsu` ($M_{04}$)
*   **Derivation formula**:
...
*   **Source square**: The 1–100 reference square ($M_{03}$) defined for the underlying transformation
*   **Derived square**: The corrected Baekjasaengseong-gyosu of `05-baekjasaengseong-gyosu` ($M_{05}$)
*   **Derivation formula**:
...
| **Mother Square (모도)**<br>Corrected Baekjayin-yang-jamo-chakjong | ![Mother Square](./figures/m06_mother.png) | The symmetric square obtained by flipping the reference square ($M_{03}$) about the horizontal axis (flipud of $M_{03}$). |
| **Generated Pure Square (순수도)**<br>Corrected Baekjasaengseong-sunsu | ![Pure Square](./figures/m04_pure.png) | The square derived by rotating the reference square ($M_{03}$) by 90 degrees about the origin (rot90 of $M_{03}$). |
| **Generated Crossed Square (교수도)**<br>Corrected Baekjasaengseong-gyosu | ![Cross Square](./figures/m05_cross.png) | The symmetric square obtained by flipping the reference square ($M_{03}$) about the vertical axis (fliplr of $M_{03}$). |
