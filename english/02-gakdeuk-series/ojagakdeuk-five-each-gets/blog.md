...
## 2. The Central Axis and the Horizontal Band

The most striking feature of the 21-point placement is that **1, 5, and 15** form the widest horizontal band (y = 2.0). This band contains 7 numbers — 1, 5, 14, 15, 18, 21, 23 — and their sum is 97.

There is also a **vertical central axis** running 19 → 6 → 20 → 5 → 17 → 2. On this axis sit 19 (Metal), 6 (Water), 20 (Earth), 5 (Earth), 17 (Fire), and 2 (Fire), so the wuxing changes continuously along the central axis.

[03_spatial_distribution.png attached]
...
| Wuxing | Numbers | Sum | Count |
|---|---|---:|---:|
| Water (水) | 1, 6, 11, 16, 21 | 55 | 5 |
| Fire (火) | 2, 7, 12, 17 | 38 | 4 |
| Wood (木) | 8, 13, 18, 23 | 62 | 4 |
...
## 5. Earth Forms the Center

Looking at the coordinates by wuxing, Earth (5, 15, 20) all lie on the vertical central axis. 20 sits between 6 and 5, 5 is the exact center of the central horizontal band, and 17 lies directly above 2.

Thus Earth can be read as the wuxing that takes charge of the center in the Cheonsu-yong-odo structure.

[05_invariants.png attached]
...
| Layer | Sum |
|---|---:|
| Upper vertex (y = 6.0) | 19 |
| Upper connector (y = 5.0–4.2) | 26 |
| Central horizontal band (y = 3.3–2.0) | 128 |
| Lower connector (y = 0.8–-0.4) | 90 |
| Lower vertex (y = -1.7) | 2 |

The central horizontal band accounts for 128, nearly half of the total sum 265. The center of the Cheonsu-yong-odo is mathematically the heaviest part.

[07_local_extensions.png attached]

## 7. Mutual Generation and Mutual Overcoming

The wuxing have two kinds of relationships: mutual generation (相生) and mutual overcoming (相克). These can be discussed from the wuxing classification itself even though the points are not connected.

- Mutual generation: Water → Wood → Fire → Earth → Metal → Water
- Mutual overcoming: Water → Fire → Metal → Wood → Earth → Water

[06_wuxing_relations.png attached]

## 8. Remaining Combinatorial Facts
