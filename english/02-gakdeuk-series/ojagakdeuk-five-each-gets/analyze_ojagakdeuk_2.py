...
# -*- coding: utf-8 -*-
"""
Ojagakdeuk (Five-Each-Gets / Heaven-Water Five-Use Diagram) Layout 2 — modern combinatorial and positional analysis

A variant with the same shape as the original placement (analyze_ojagakdeuk.py)
but a different number placement.
Central line (top to bottom): 12, 5, 2, 15, 6, 1, 16
Left (outer column to inner column, vertical): 7 / 13, 3, 11 / 17, 20, 10
Right (outer column to inner column, vertical): 8 / 19, 4, 14 / 18, 9, 21
Analysis target: a diagram placing all of the numbers 1–21 (21 numbers) in the
Cheonsu-yong-odo form.
...
# ============================================================
# 1. Original data structure (Layout 2: 1–21)
# ============================================================
...
# Wuxing colors that are visually distinct (fixes the duplicate gray of
# Water/Metal in the original STYLE)
RESIDUE_STYLE: dict[int, dict[str, str]] = {
    1: {"face": "#CFE2F5", "edge": "#4488CC", "name": "Water", "en": "Water"},
    2: {"face": "#F4D0D0", "edge": "#CC4444", "name": "Fire", "en": "Fire"},
    3: {"face": "#D4ECD4", "edge": "#44AA44", "name": "Wood", "en": "Wood"},
    4: {"face": "#E8E8E8", "edge": "#888888", "name": "Metal", "en": "Metal"},
    0: {"face": "#F6E5A3", "edge": "#CC9944", "name": "Earth", "en": "Earth"},
}
...
def validate() -> None:
    ...
    assert set(all_values) == set(range(1, 22)), "uses all of 1–21"
    assert sum(all_values) == 231, "total sum is 231"
...
print("=" * 60)
print("Ojagakdeuk (Five-Each-Gets / Heaven-Water Five-Use Diagram) Layout 2")
...
draw_nodes(ax)
ax.set_title(
    "Ojagakdeuk (Five-Each-Gets / Heaven-Water Five-Use Diagram) Layout 2 — Original placement\n"
    "1–21 · mod 5 wuxing classification · Cheonsu-yong-odo geometric structure",
    fontsize=16,
    fontweight="bold",
)
ax.set_xlim(-5.0, 5.0)
...
save_fig("01_original_graph_2.png")
plt.close()
...
plt.suptitle("Wuxing (five-phase) subgroup decomposition (Layout 2)", fontsize=16, fontweight="bold", y=1.02)
plt.tight_layout()
save_fig("02_wuxing_decomposition_2.png")
plt.close()
...
ax.set_title("mod 5 residue spatial distribution (Layout 2)", fontsize=13, fontweight="bold")
...
ax.set_title("Coordinate distribution by wuxing (Layout 2)", fontsize=13, fontweight="bold")
ax.legend(loc="lower right", fontsize=10)
ax.axis("off")

plt.tight_layout()
save_fig("03_spatial_distribution_2.png")
plt.close()
...
ax.set_title("Left · Center · Right symmetric distribution (Layout 2)", fontsize=13, fontweight="bold")
ax.axis("off")
...
ax.set_title("Sum by horizontal level (Layout 2)", fontsize=13, fontweight="bold")
...
plt.tight_layout()
save_fig("04_symmetry_analysis_2.png")
plt.close()
...
ax.set_title("Sum by wuxing (55, 38, 42, 46, 50)", fontsize=12, fontweight="bold")
...
ax.set_title("Left · Center · Right symmetric sums (Left=81, Center=57, Right=93)", fontsize=12, fontweight="bold")
...
ax.axis("off")
plt.tight_layout()
save_fig("06_wuxing_relations_2.png")
plt.close()
...
ax.set_title("Vertical Layer Sum Distribution (Layout 2)", fontsize=13, fontweight="bold")
...
plt.tight_layout()
save_fig("07_rule_variation_model_2.png")
plt.close()
...
ax.set_title("Horizontal Level Sums and Node Counts (Layout 2)", fontsize=13, fontweight="bold")
ax.set_ylabel("Sum", fontsize=10)
...
ax.set_title("Left · Center · Right symmetric sums (Layout 2)", fontsize=13, fontweight="bold")
ax.set_ylabel("Sum", fontsize=10)
for bar, val in zip(ax.patches, components.values()):
    ax.text(
        bar.get_x() + bar.get_width() / 2,
...
plt.tight_layout()
save_fig("08_position_patterns_2.png")
plt.close()

print("\n" + "=" * 60)
print("All images generated successfully! (Layout 2)")
