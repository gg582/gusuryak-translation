...
# Display labels for the nine palaces.
PALACE_LABELS: Final = {
    "upper_left":   "Upper-left palace",
    "upper_center": "Upper-center palace",
    "upper_right":  "Upper-right palace",
    "middle_left":  "Middle-left palace",
    "center":       "Center palace",
    "middle_right": "Middle-right palace",
    "lower_left":   "Lower-left palace",
    "lower_center": "Lower-center palace",
    "lower_right":  "Lower-right palace",
}
...
def find_cjk_font() -> FontProperties:
    ...
    raise RuntimeError(
        "Could not find a CJK font. "
        "Please install Noto Sans CJK KR or NanumGothic."
    )
...
def validate() -> None:
    ...
    if sorted(numbers) != list(range(1, 34)):
        raise ValueError("Numbers 1 through 33 must appear exactly once.")

    if sum(numbers) != 561:
        raise ValueError("The sum of numbers 1 through 33 must be 561.")
    ...
    for palace_name, coordinates in PALACES.items():
        ...
        if palace_sum != TARGET_SUM:
            raise ValueError(
                f"The sum of {palace_name} is {palace_sum}. "
                f"It must be {TARGET_SUM}."
            )
    ...
    if actual_counts != RESIDUE_COUNTS:
        raise ValueError(
            f"Residue group counts do not match the notebook: {actual_counts}"
        )
    ...
    if repeated_total != TOTAL_SUM:
        raise ValueError(
            f"The repeated-count sum of the 9 palaces is {repeated_total}. "
            f"It must be {TOTAL_SUM}."
        )
...
def draw_palace_boundaries(ax: Axes, font: FontProperties) -> None:
    ...
    for palace_name, (x, y) in PALACE_CENTERS.items():
        ...
        ax.text(
            x,
            label_y,
            f"{PALACE_LABELS[palace_name]}  sum 85",
            ha="center",
            va="center",
            fontproperties=font,
            fontsize=9.5,
...
def draw_legend(ax: Axes, font: FontProperties) -> None:
    ...
    summary = (
        "Nodes: 1–33, each once",
        "Palaces: 9",
        "Each palace: cross-shaped 5 nodes",
        "Each palace sum: 85",
        "9-palace repeated-count sum: 85 × 9 = 765",
    )
...
def draw(
    ...
    graph_ax.set_title(
        "Nakseo-ogudo — 9 palaces · 5 nodes · each sum 85",
        fontproperties=font,
        fontsize=18,
        pad=18,
    )
